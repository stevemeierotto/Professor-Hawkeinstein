...
## ⚠️ IMPORTANT: Legacy System Documentation

**This document describes the original Agent Factory system (web scraping + knowledge base approach).** This approach has been superseded by the current **5-agent automated course generation pipeline** implemented in December 2025 - January 2026.

### Current System (Use This)
- **📄 Primary Reference**: [COURSE_GENERATION_ARCHITECTURE.md](COURSE_GENERATION_ARCHITECTURE.md) — Complete 5-agent pipeline specification
- **📄 API Guide**: [COURSE_GENERATION_API.md](COURSE_GENERATION_API.md) — Endpoints for standards, outlines, lessons, questions, assessments
- **📄 Assessment API**: [ASSESSMENT_GENERATION_API.md](ASSESSMENT_GENERATION_API.md) — Unit tests, midterms, finals generation

**Current workflow:** Grade + Subject → LLM-generated standards → Outline → Lesson content → Questions → Validation  
**Content source:** All content is **LLM-generated** (no web scraping). Standards may reference external frameworks but lesson content is created by the local LLM.

### This Document (Legacy)
The system described below was used for manual content curation and specialized agent creation with RAG knowledge bases. It remains documented for:
- Understanding the evolution of the platform
- Reference for scraping functionality (still present but not primary workflow)
- Historical context for `scraped_content`, `content_reviews`, and `training_exports` tables

**For course creation in 2026**, use the automated 5-agent pipeline documented in the files listed above.

---

## Original Overview (Legacy System)
