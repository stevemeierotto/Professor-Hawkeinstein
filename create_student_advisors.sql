-- Create student_advisors table for advisor assignment
...
-- Insert Professor Hawkeinstein agent if not exists
INSERT IGNORE INTO agents (agent_id, agent_name, agent_type, specialization, system_prompt, model_name, temperature, is_active)
VALUES (
    1,
...
-- Create advisor assignment for existing students (if any)
INSERT INTO student_advisors (student_id, advisor_type_id, is_active, conversation_history, testing_results)
SELECT user_id, 1, 1, '[]', '[]'
FROM users
WHERE role = 'student'
AND user_id NOT IN (SELECT student_id FROM student_advisors)
ON DUPLICATE KEY UPDATE advisor_type_id = advisor_type_id;
