...
**Standards:** Retrieved from CSP (Common Standards Project) API as input for generation.

```
Standards (CSP API) → Agent 1 (Outline) → Agent 2 (Lessons) → Agent 3 (Questions)
                                                                  ↓
...
### 4. CSP Standards Retrieval (Reference Data Only)
**File:** `api/admin/scraper_csp.php` (misnomer - retrieves via API, doesn't scrape)

**Two-step process:**
1. GET jurisdiction data: `/jurisdictions/{jurisdictionId}`
2. GET standards: `/standard_sets/{standardSetId}`

**Purpose:** Standards are used as INPUT for LLM content generation, not as content themselves.
The LLM reads the standards and generates original lesson content, not copying from any source.

**Storage pattern:** Standards stored for reference during course creation.
```php
storeCSPStandards($standards, $retrievedBy) {
  // Stores standards as reference data:
  // - title: "Alaska - Grade 1 Mathematics Standards (94 items)"
  // - content_text: All standard descriptions
  // - content_html: Formatted HTML for display
  // - metadata: JSON array of standard objects
}
```

**Note:** Standards are just prompts/instructions for the LLM. All actual lesson content is AI-generated.
...
**List educational content (admin):**
```php
// Returns: content_id, url, title, content_preview (SUBSTRING of content_text)
// All content is AI-generated, url field used for internal reference
// content_type: 'ai_generated' for all lesson content
```
...
- `POST /api/admin/scraper_csp.php` - Retrieve CSP standards via API (returns combined entry)
- `GET /api/admin/list_scraped_content.php` - Returns stored standards with pagination (legacy endpoint)
...
│   │   ├── scraper_csp.php        # CSP standards retrieval (via API)
